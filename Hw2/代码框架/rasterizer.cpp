...
{   
	...
	// 判断叉乘方向是否朝外（Z轴） a_x*b_y-a_y*b_x为Z轴系数
    bool ans = AB[0] * AP[1] - AB[1] * AP[0] > 0 && BC[0] * BP[1] - BC[1] * BP[0] > 0 && CA[0] * CP[1] - CA[1] * CP[0] > 0;
	return ans;
}
    ...
    if(MSAA){
        ...
        for(int x = minX; x <= maxX; ++x){
            for(int y = minY; y <= maxY; ++y){
                ...
                int count = 0;
                for(int i = 0; i < 4; ++i){
                    if(insideTriangle(static_cast<float>(x) + pos[i][0], static_cast<float>(y) + pos[i][1], t.v)){
                        //获取Z深度
                        auto tup = computeBarycentric2D(static_cast<float>(x) + pos[i][0], static_cast<float>(y) + pos[i][1], t.v);
                        float alpha;
                        float beta;
                        float gamma;
